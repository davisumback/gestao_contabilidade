...
<script type="text/javascript">
    $('#edita-cnae').on('show.bs.modal', function (event) {
        ...
        var cnae = button.data('cnae'); // Extract info from data-* attributes
        ...
        $('#editaCnae').mask('0000-0/00');
    });    

    $('#deleta-cnae').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget) // Button that triggered the modal
        var idCnae = button.data('cnae-id'); // Extract info from data-* attributes
        // var cnaePrincipal = button.data('cnae-principal'); // Extract info from data-* attributes
        // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
        // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
        var modal = $(this)
        modal.find('#idDeletaCnae').val(idCnae);
        // modal.find('#cnaePrincipal').val(cnaePrincipal);
        // modal.find('#editaCnae').val(cnae);
    });
