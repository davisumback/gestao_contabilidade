    ...
    <?php if (!array_key_exists('enviadas', $_GET)) : ?>
        <div class="table-responsive mb-5">
            <table id="myTable" class="table table-bordered table-hover">
                <?php if (in_array($tipoGuia, $guiasPresumido) || $tipoGuia == 'DAS') : ?>
                    ...
                    <tbody class="label-cadastro <?=$corStatusGuias?>">
                        <?php foreach ($empresas as $empresa) : ?>
                            <tr>
                                <td><?=$empresa['id']?></td>
                                <td><?=$empresa['nome_empresa']?></td>
                                <td class="text-success" id='<?=$empresa['id']?>'>
                                    <?php
                                        // if ($empresa['regime_tributario'] == 'SN') {
                                        //     $empresaDao = new EmpresaDAO();
                                        //     $retorno = $empresaDao->getProlabore($empresa['id'], $competenciaAnterior);
                                        //
                                        //     if ($retorno['updated_at'] != null) {
                                        //         echo '<span class=\'text-danger\'>' . Helpers::formataMoedaView($retorno['prolabore']) . '</span>';
                                        //     } else {
                                        //         echo Helpers::formataMoedaView($retorno['prolabore']);
                                        //     }
                                        // }
                                    ?>
                                </td>
                                <td class="text-success">
                                    <?php
                                        // if ($empresa['regime_tributario'] == 'SN') {
                                        //     $empresaDao = new EmpresaDAO();
                                        //     $retorno = $empresaDao->getProlabore($empresa['id'], $dataCompetencia);
                                        //
                                        //     if ($retorno['updated_at'] != null) {
                                        //         echo '<span class=\'text-danger\'>' . Helpers::formataMoedaView($retorno['prolabore']) . '</span>';
                                        //     }else {
