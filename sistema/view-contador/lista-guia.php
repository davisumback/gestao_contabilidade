...
$guiaDao = new GuiaDAO();
$empresas = $guiaDao->getGuiasEnviadasPorContador($tipoGuia, $dataCompetencia, $usuariosId);
...
if (!array_key_exists('enviadas', $_GET)) {
    ...
    if (in_array($tipoGuia, $guiasSN)) {
        $empresas = $guiaDao->getGuiasPendentesContador($tipoGuia, $dataCompetencia, 'SN', $_SESSION['id_usuario']);
    }

    if (in_array($tipoGuia, $guiasPresumido)) {
        $empresas = $guiaDao->getGuiasPendentesContador($tipoGuia, $dataCompetencia, 'Presumido', $_SESSION['id_usuario']);
    }
}
?>
...
<?php if (empty($empresas)) : ?>
    <div class="alert alert-light text-center pt-4 pb-4 font-weight-bold <?= $corTitulo ?>" role="alert">
        <?= $titulo ?>
    </div>
